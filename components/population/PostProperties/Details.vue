<template>
  ...
  <div class="mx-4 px-4 md:px-8 sm:grid sm:grid-cols-3 sm:mx-auto gap-4 max-w-[995px]">
    ...
    <div class="col-span-3">
      <label for="otherDetails" class="mb-2">Otras amenidades</label>
      <select class="form-control col-span-3 sm:mb-2 mb-5" id="otherDetails">
        <option v-for="(detail) in otherDetails" :key="detail" class="option-label">
        {{ detail }}
        </option>
      </select>
    </div>
    <div class="col-span-3 w-full gap-4 sm:flex sm:mb-2 mb-5">
      <label class="w-full mb-5 sm:mb-0">
        ...
        <input class="form-control" placeholder="Metros²" type="text">
      </label>
      <label class="w-full">
        Superficie de total
        <input class="form-control" placeholder="Metros²" type="text">
